...
public class UpdateVendor extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int status=vendao.updateVendor(ven);
		
		
		if(status==1)
		{
			RequestDispatcher rd=request.getRequestDispatcher("index.jsp");
			pw.println("<span style=color:green;font-size:20px;>Vendor Updated Successfully</span>");
			rd.include(request, response);
		}
		else
		{
			RequestDispatcher rd=request.getRequestDispatcher("UpdateVendor.jsp");
			pw.println("<span style=color:red;size:20px>Failed Updating Vendor</span>");
			rd.include(request, response);
		}
