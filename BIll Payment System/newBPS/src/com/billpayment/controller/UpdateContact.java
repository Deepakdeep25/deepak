...
public class UpdateContact extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int status=cudao.updateContact(userId,Contact);
		if(status==1){
			pw.println("<span style=color:green;font-size:20px;>Contact Details Updated Successfully</span><br>");
			RequestDispatcher rd=request.getRequestDispatcher("BillMapping.jsp");
			rd.include(request, response);
			}
		else
		{
			pw.println("<span style=color:red;size:20px>failed updating your Contact Number</span>");
